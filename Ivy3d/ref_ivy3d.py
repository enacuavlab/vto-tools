...
#    if(dum2==2):self.stop_flag=0
  ...
  def on_msg_ground_ref_ltp_enu(self, *larg):
    ...
    self.model_update(acid,
        ...
        ((np.array([100*pos[0]+70,100*pos[2]+5,-100*pos[1]-300],dtype=float)), # y,z,-x
         (np.array([quat[0],quat[1],-quat[3],-quat[2]],dtype=float)))) # qi qx qy qz
  ...
  def model_update(self,acid,quatinit,curr):
    ...
    for i,elt in enumerate(self.meshtab):
      if(acid==elt.acid):
        ...
        if any(abs(abs(curr[1][i])-abs(elt.transf[1][i]))>0.008 for i in range(4)):
          print("updateRot")
          elt.mesh.rotate(R=elt.mesh.get_rotation_matrix_from_quaternion(
           ((R.from_quat(curr[1]))*(R.from_quat(elt.transf[2]))*(R.from_quat([1.,0.,0.,0.]))).as_quat()),
                          center=elt.mesh.get_center())
          elt.transf[1][:]=curr[1]
          elt.transf[2][:]=(R.from_quat(curr[1]).inv()).as_quat()
          updateFlag=True
          flag=True
    ...
    if notregistered:
      ...
      mesh_model.rotate(
        R=mesh_model.get_rotation_matrix_from_quaternion(
            ((R.from_quat(curr[1]))*(R.from_quat(quatinit))).as_quat()),
        center=mesh_model.get_center())
      self.meshtab.append(
        Model(acid,mesh_model,(curr[0],curr[1],((R.from_quat(curr[1])).inv()).as_quat())))
