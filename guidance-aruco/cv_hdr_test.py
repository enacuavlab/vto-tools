...
#    camera.awb_mode='off'
#    camera.awb_gains = (Fraction(379, 256), Fraction(253, 128))

#    camera.awb_mode='off'
#    camera.awb_gains = (Fraction(379, 256), Fraction(253, 128))
    camera.shutter_speed = 198
    self.storeimg = []
...
#    self.alignMTB = cv2.createAlignMTB()
  ...
  def analyse(self, array):
    ...
    if markersIds == None:
      ...
      if(self.notfound<1):self.notfound=self.notfound+1
      else:

        self.storeimg.append(gray1)

        self.notfound=0
        if len(self.storeimg)==2:
#          print("in")
#          self.alignMTB.process(self.storeimg, self.storeimg)
          res_mertens = self.mergeMertens.process(self.storeimg)
          res_mertens_8bit = np.clip(res_mertens*255, 0, 255).astype('uint8')
#          print("out")
          self.strOut.write(res_mertens_8bit)
          self.storeimg.pop(0)

        if camera.exposure_speed != 198:camera.shutter_speed=198
        else:camera.shutter_speed=33243
...
#    self.strOut.write(gray1)
    print(camera.exposure_speed,camera.shutter_speed)
