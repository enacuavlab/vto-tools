 ...
 * @param _cpu_time 
 */
static inline void pprzlink_msg_v2_send_ROVER_STATUS(struct pprzlink_msg * msg, uint8_t *_rc_status, uint8_t *_frame_rate, uint8_t *_gps_status, uint8_t *_ap_mode, uint8_t *_ap_motors_on, uint16_t *_vsupply, uint16_t *_cpu_time) {
#if PPRZLINK_ENABLE_FD
  long _FD = 0; /* can be an address, an index, a file descriptor, ... */
#endif
  const uint8_t size = msg->trans->size_of(msg, /* msg header overhead */4+1+1+1+1+1+2+2);
  if (msg->trans->check_available_space(msg, _FD_ADDR, size)) {
    msg->trans->count_bytes(msg, size);
    msg->trans->start_message(msg, _FD, /* msg header overhead */4+1+1+1+1+1+2+2);
...
#define DOWNLINK_SEND_ROVER_STATUS(_trans, _dev, rc_status, frame_rate, gps_status, ap_mode, ap_motors_on, vsupply, cpu_time) pprz_msg_send_ROVER_STATUS(&((_trans).trans_tx), &((_dev).device), AC_ID, rc_status, frame_rate, gps_status, ap_mode, ap_motors_on, vsupply, cpu_time)
/**
 ...
 * @param _cpu_time 
 */
static inline void pprz_msg_send_ROVER_STATUS(struct transport_tx *trans, struct link_device *dev, uint8_t ac_id, uint8_t *_rc_status, uint8_t *_frame_rate, uint8_t *_gps_status, uint8_t *_ap_mode, uint8_t *_ap_motors_on, uint16_t *_vsupply, uint16_t *_cpu_time) {
    ...
    msg.receiver_id = 0;
    msg.component_id = 0;
    pprzlink_msg_v2_send_ROVER_STATUS(&msg,_rc_status,_frame_rate,_gps_status,_ap_mode,_ap_motors_on,_vsupply,_cpu_time);
}


#else // DOWNLINK

#define DOWNLINK_SEND_ROVER_STATUS(_trans, _dev, rc_status, frame_rate, gps_status, ap_mode, ap_motors_on, vsupply, cpu_time) {}
static inline void pprz_send_msg_ROVER_STATUS(struct transport_tx *trans __attribute__((unused)), struct link_device *dev __attribute__((unused)), uint8_t ac_id __attribute__((unused)), uint8_t *_rc_status __attribute__((unused)), uint8_t *_frame_rate __attribute__((unused)), uint8_t *_gps_status __attribute__((unused)), uint8_t *_ap_mode __attribute__((unused)), uint8_t *_ap_motors_on __attribute__((unused)), uint16_t *_vsupply __attribute__((unused)), uint16_t *_cpu_time __attribute__((unused))) {}
...
/** Getter for field vsupply in message ROVER_STATUS
  ...
  * @return 
  */
static inline uint16_t pprzlink_get_DL_ROVER_STATUS_vsupply(uint8_t * _payload __attribute__((unused)))
{
