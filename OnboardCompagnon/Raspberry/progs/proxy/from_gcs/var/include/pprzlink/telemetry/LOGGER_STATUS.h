 ...
 */
static inline void pprzlink_msg_v2_send_LOGGER_STATUS(struct pprzlink_msg * msg, uint8_t *_status, uint8_t *_errno, uint32_t *_used) {
...
#define DOWNLINK_SEND_LOGGER_STATUS(_trans, _dev, status, errno, used) pprz_msg_send_LOGGER_STATUS(&((_trans).trans_tx), &((_dev).device), AC_ID, status, errno, used)
/**
 ...
 * @param _used Accumulated number of bytes written by the logger
 */
static inline void pprz_msg_send_LOGGER_STATUS(struct transport_tx *trans, struct link_device *dev, uint8_t ac_id, uint8_t *_status, uint8_t *_errno, uint32_t *_used) {
    ...
    msg.receiver_id = 0;
    msg.component_id = 0;
    pprzlink_msg_v2_send_LOGGER_STATUS(&msg,_status,_errno,_used);
}


#else // DOWNLINK

#define DOWNLINK_SEND_LOGGER_STATUS(_trans, _dev, status, errno, used) {}
static inline void pprz_send_msg_LOGGER_STATUS(struct transport_tx *trans __attribute__((unused)), struct link_device *dev __attribute__((unused)), uint8_t ac_id __attribute__((unused)), uint8_t *_status __attribute__((unused)), uint8_t *_errno __attribute__((unused)), uint32_t *_used __attribute__((unused))) {}
