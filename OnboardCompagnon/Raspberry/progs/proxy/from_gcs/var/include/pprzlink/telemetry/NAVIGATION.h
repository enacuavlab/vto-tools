 ...
 * @param _oval_count 
 */
static inline void pprzlink_msg_v2_send_NAVIGATION(struct pprzlink_msg * msg, uint8_t *_cur_block, uint8_t *_cur_stage, float *_pos_x, float *_pos_y, float *_dist_wp, float *_dist_home, uint8_t *_circle_count, uint8_t *_oval_count) {
#if PPRZLINK_ENABLE_FD
  long _FD = 0; /* can be an address, an index, a file descriptor, ... */
#endif
  const uint8_t size = msg->trans->size_of(msg, /* msg header overhead */4+1+1+4+4+4+4+1+1);
  if (msg->trans->check_available_space(msg, _FD_ADDR, size)) {
    msg->trans->count_bytes(msg, size);
    msg->trans->start_message(msg, _FD, /* msg header overhead */4+1+1+4+4+4+4+1+1);
...
static inline void pprz_msg_send_NAVIGATION(struct transport_tx *trans, struct link_device *dev, uint8_t ac_id, uint8_t *_cur_block, uint8_t *_cur_stage, float *_pos_x, float *_pos_y, float *_dist_wp, float *_dist_home, uint8_t *_circle_count, uint8_t *_oval_count) {
    ...
    msg.receiver_id = 0;
    msg.component_id = 0;
    pprzlink_msg_v2_send_NAVIGATION(&msg,_cur_block,_cur_stage,_pos_x,_pos_y,_dist_wp,_dist_home,_circle_count,_oval_count);
}
