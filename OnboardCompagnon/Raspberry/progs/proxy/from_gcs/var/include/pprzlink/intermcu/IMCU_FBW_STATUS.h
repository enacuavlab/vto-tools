 ...
 * @param _current 
 */
static inline void pprzlink_msg_v2_send_IMCU_FBW_STATUS(struct pprzlink_msg * msg, uint8_t *_mode, uint8_t *_rc_status, uint8_t *_frame_rate, uint16_t *_vsupply, int32_t *_current) {
#if PPRZLINK_ENABLE_FD
  long _FD = 0; /* can be an address, an index, a file descriptor, ... */
#endif
  const uint8_t size = msg->trans->size_of(msg, /* msg header overhead */4+1+1+1+2+4);
  if (msg->trans->check_available_space(msg, _FD_ADDR, size)) {
    msg->trans->count_bytes(msg, size);
    msg->trans->start_message(msg, _FD, /* msg header overhead */4+1+1+1+2+4);
    ...
    msg->trans->put_bytes(msg, _FD, DL_TYPE_INT32, DL_FORMAT_SCALAR, (void *) _current, 4);
...
/** Getter for field vsupply in message IMCU_FBW_STATUS
  ...
  * @return 
  */
static inline uint16_t pprzlink_get_DL_IMCU_FBW_STATUS_vsupply(uint8_t * _payload __attribute__((unused)))
{
...
/** Getter for field current in message IMCU_FBW_STATUS
  ...
  * @return 
  */
static inline int32_t pprzlink_get_DL_IMCU_FBW_STATUS_current(uint8_t * _payload __attribute__((unused)))
{
